...
token = '6335773598:AAHUjNxrs9VzNfOOgp5waUX2cnMIGdVS44E'
...
الرجاء ارسال ملف صالح
  ...
  ''',parse_mode='html')
  return
 with open('proxyhit.txt', "r") as kf:
  jf = kf.readlines()
  pro= random.choice(jf)
  try:
  	porrt = pro.split(':')[1]
  except:
  	bot.send_message(z.chat.id,text='حصل خطا في البروكسيات\nاذا استمرت المشكله راسل @d_xiim')
 proxy = {
	'http': f'http://{pro}',
	}
 with open(filename, 'r') as f1:
 ...
 while True:
  ...
  req = requests.post(url, headers=headers, data=data,proxies=proxy).text
  ...
  'f.req': f'["AEThLlxesmDPjlIUtsKkl4gagqph3sUY0IZmfw5lagvqpB1Lv36xsnFGRLoBooKieRg9-l5BgLByLWnE8iVWy5LhKSsm6500ujWsSYJ2Qlk4sSaCofqpx2baRre5kKwBa5m5I3o3RUuK3b_tBDNxI7JcmD2FcZgvq60lA7ZuV6qaHIHGtSnLwtp9AEOy8Xg9bMNP_vI73fXYqmGNtwnLkgr-EfASQx8tHQ","","","{ss}",true,"S2131635123:1683406620139000",1]',
   ...
   res= requests.post(url2, headers=head1, data=data, proxies=proxy).text
   ...
   if skf=='1':
    ...
    'X-Requested-With': 'XMLHttpRequest',}
   	r5 = requests.get(url5,headers=headers5,proxies=proxy).json()
   	try:
